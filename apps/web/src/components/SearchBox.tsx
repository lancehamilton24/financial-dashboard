...
type Props = { activeIndex: number; error: string | null; isLoading: boolean; query: string; results: SymbolSearchResult[]; searched: boolean; onActiveIndexChange: (index: number) => void; onQueryChange: (query: string) => void; onSearch: () => void; onSelect: (result: SymbolSearchResult) => void };

export function SearchBox({ activeIndex, error, isLoading, query, results, searched, onActiveIndexChange, onQueryChange, onSearch, onSelect }: Props) {
